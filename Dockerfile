...
# to always fallback to standard driveterra theme.
# ------------------------------------------------------------------------------------
ARG VERSION=23.0
FROM registry.access.redhat.com/ubi9 AS ubi-micro-build
MAINTAINER support@driveterra.io
...
# Install driveterra theme
ADD --chown=keycloak:keycloak build/image/driveterra-theme.jar /opt/keycloak/providers
...
ENV KC_DB_URL_DATABASE=driveterra
...
ENV KEYCLOAK_DEFAULT_THEME=driveterra
ENV KEYCLOAK_ACCOUNT_THEME=driveterra
...
ENTRYPOINT /opt/keycloak/bin/kc.sh ${KEYCLOAK_START_COMMAND:-start} --optimized --spi-theme-login-default=${KEYCLOAK_LOGIN_THEME:-driveterra} --spi-theme-account-theme=${KEYCLOAK_ACCOUNT_THEME:-driveterra} --spi-theme-welcome-theme=${KEYCLOAK_WELCOME_THEME:-keycloak} --spi-theme-admin-theme=${KEYCLOAK_ADMIN_THEME:-keycloak} ${KEYCLOAK_START_OPTS:-}
